...
def scan_file(sample):
    scanner = subprocess.Popen(['clamscan', '-'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    stdout, stderr = scanner.communicate(sample.read())
    output = stdout.splitlines()
    return output

def parse_output(output):
    response = {'engine': 'ClamAV'}
    pest_name = re.match(b'.+: (.+) FOUND', output[0])
    if pest_name:
        response['pest_name'] = pest_name.group(1).decode(encoding='utf-8')
    else:
        response['pest_name'] = None
    engine_version = re.match(b'Engine version: (.+)', output[4])
    if engine_version:
